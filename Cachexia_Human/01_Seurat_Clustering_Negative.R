...
S70 <- Read10X_h5("cellbender_out/S70_filtered.h5", use.names = TRUE, unique.features = TRUE)
S72 <- Read10X_h5("cellbender_out/S72_filtered.h5", use.names = TRUE, unique.features = TRUE)
S74 <- Read10X_h5("cellbender_out/S74_filtered.h5", use.names = TRUE, unique.features = TRUE)

S70_obj <- CreateSeuratObject(counts = S70,min.features = 100)
S72_obj <- CreateSeuratObject(counts = S72,min.features = 100)
S74_obj <- CreateSeuratObject(counts = S74,min.features = 100)

S70_obj@meta.data$Sample <- "Neg_WS_PDAC"
S72_obj@meta.data$Sample <- "Neg_Control"
S74_obj@meta.data$Sample <- "Neg_C_PDAC"

seuObject <- merge(S70_obj,
                   y = c(S72_obj,S74_obj),
                   add.cell.ids = c("S70","S72","S74"),
                   project = "Gutt_human")
...
#load("output_sct_Integrated_Negative/01_SeuratObj_Unfilt_YFSex.RData")
...
seuObject_split <- seuObject_split[c("S70","S72","S74")]
...
save(seuObject_integrated, file = "output_sct_Integrated_Negative/03_SeuratObj_Integrated_allres.RData")
