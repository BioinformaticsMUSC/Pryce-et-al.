...
library(ggvolc) 
library(dplyr, include.only = c('rename', 'filter')) 
...
output$sc1c2oup.png <- downloadHandler( 
  ...
  observeEvent(input$sc1m_all_select, { 
  #print(input$sc1m_all_select) 
}) 

output$sc1m_all.ui <- renderDataTable ( 
  subset(sc1m_all, cluster == input$sc1m_all_select), filter="top" 
) 
...
output$gsig_download.png <- downloadHandler( 
  ...
  scVolc <- function(conf, genes, de_selection, genes_selection, de_subset_col, de_subset, fold_change, p_value, subset_toggle, 
                     top10_toggle, xlim_down, xlim_up, ylim) { 
    ...
    if(top10_toggle == TRUE) { 
      ...
      genes_selection <- rbind(top_pos, top_neg) 
    } 
    
    ggOut <- ggvolc(genes, genes_selection, fc=fold_change, p_value=p_value, size_var="set_size") + labs(title=de_selection) + xlim(xlim_down, xlim_up) + ylim(0, ylim) 
    
    return(ggOut) 
  } 
  
  output$volc_subset.ui <- renderUI({ 
    subset = levels(sc1de_genes_ggvolc[[input$volc_subset]]) # should you find a way to encode this into the sc1conf file?
    checkboxGroupInput("volc_subset2", "Select which subsets to show", inline = TRUE, 
                       choices = subset, selected = subset) 
  }) 
  
  observeEvent(input$volc_subset_sel_all, { 
    subset = levels(sc1de_genes_ggvolc[[input$volc_subset]]) 
  ...
  output$volc_plot <- renderPlot({ 
    if(sc1de_genes_ggvolc[1,1] == "ERROR") { 
      ...
      scVolc(sc1conf, sc1de_genes_ggvolc, input$volc_de_select, genes_selection=NULL, input$volc_subset, input$volc_subset2, 
             input$volc_fold_change, input$volc_p_value, input$volc_subset_toggle, input$volc_top10_toggle, input$volc_xlim_down, input$volc_xlim_up, input$volc_ylim) 
    } 
  }) 
  output$volc_download.pdf <- downloadHandler( 
    ...
    content = function(file) { ggsave( 
      ...
      plot = scVolc(sc1conf, sc1de_genes_ggvolc, input$volc_de_select, genes_selection=NULL, input$volc_subset, input$volc_subset2, 
             input$volc_fold_change, input$volc_p_value, input$volc_subset_toggle, input$volc_top10_toggle, input$volc_xlim_down, input$volc_xlim_up, input$volc_ylim) 
      ) 
    } 
  ) 
