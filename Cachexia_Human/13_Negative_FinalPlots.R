...
dir.create("output_Figure_Negative")
...
#write.table(col, "output_Figure_Negative/Colors_Used.txt", sep="\t",quote=F)
...
pdf("output_Figure_Negative/08_UMAP_Labelled_Alternative.pdf", width = 7, height = 6)
...
pdf("output_Figure_Negative/09_UMAP_Labelled_Alternative_withlegend_withaxis.pdf", width = 6, height = 5)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
...
pdf("output_Figure_Negative/09_UMAP_Labelled_Alternative_withlegend_withaxis_splitby.pdf", width = 18, height = 6)
df %>%
...
save(neg_subfaps, file = "output_Figure_Negative/04_SubsetFaps_Reclustered.RData")
load("output_Figure_Negative/04_SubsetFaps_Reclustered.RData")
...
pdf("output_Figure_Negative/10_Faps_SubsetUMAP.pdf", width = 8, height = 4)
...
pdf("output_Figure_Negative/11_Faps_SubsetUMAP_Splitby_Genotype.pdf", width = 12, height = 4)
...
openxlsx::write.xlsx(all_markers.Sign, 
                     file = "output_Figure_Negative/Faps_MarkersbyCluster_padjLT05_logfcGT0.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Markers")
...
#pdf("output_Figure_Negative/12_Faps_SubsetUMAP_ViolinStacked.pdf", width = 10, height = 20)
#VlnPlot(neg_subfaps, genes, stack = TRUE, sort = TRUE, flip = TRUE) +
#  theme(legend.position = "none") + ggtitle("Identity on x-axis")
#dev.off()

pdf("output_Figure_Negative/13_Faps_SubsetUMAP_Violin_1.pdf", width = 20, height = 15)
VlnPlot(neg_subfaps, genes,ncol = 3,pt.size = 0)
dev.off()

#requested on Oct10
genes <- c("MMP3", "PLAU", "TGIF1", "MYC")
pdf("output_Figure_Negative/13_Faps_SubsetUMAP_Violin_2.pdf", width = 20, height = 5)
...
# Make the plot
pdf("output_Figure_Negative/14_Faps_SubsetUMAP_DonutPlot.pdf", width = 12, height = 4)
p1 + p3 + p2
dev.off()
...
pdf("output_Figure_Negative/15_FAPs_FinalUMAP_withlegend_withaxis.pdf", width = 5, height = 4)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
...
pdf("output_Figure_Negative/16_FAPs_FinalUMAP_withlegend_withaxis_splitby.pdf", width = 18, height = 6)
df %>%
  mutate(Geno_ordered=fct_relevel(Sample,geno_order)) %>%
 ggplot(aes(x=UMAP_1, y=UMAP_2)) +
