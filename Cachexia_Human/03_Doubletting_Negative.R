...
save(seuObject_integrated_slim, file = "output_sct_Integrated_Negative/05_SeuratObj_SCT_slim_withDoublets.RData")

df <- seuObject_integrated_slim@meta.data %>% as.data.frame()
tmp <- table(df$Doublets,df$Sample) %>% as.data.frame()
tmp

pdf("output_sct_Integrated_Negative/09_GeneExp_Singlet-Doublet_Sample.pdf",width = 8,height = 6)
...
save(seuObject_slim_nodoub, file = "output_sct_Integrated_Negative/06_SeuratObj_SCT_30pcs_06res_NoDoublet_Reclustered.RData")
...
save(seuObject_slim_nodoub_slim, file = "output_sct_Integrated_Negative/06_SeuratObj_SCT_30pcs_06res_NoDoublet_Reclustered_slim.RData")



pdf("output_sct_Integrated_Negative/09_UMAP_NoDoublets_Reclustered.pdf", width = 13, height = 6)
...
seuObject_slim_nodoub_withHarmony <- FindClusters(seuObject_slim_nodoub_withHarmony, resolution = c(0.3,0.4,0.5,0.6), 
                                                  algorithm = 1, n.iter = 1000,graph.name = "SCT_snn")

head(seuObject_slim_nodoub_withHarmony)

pdf("output_sct_Integrated_Negative/10_UMAP_NoDoublets_Reclustered_AfterHarmony_Res07.pdf", width = 13, height = 6)
...
pdf("output_sct_Integrated_Negative/10_UMAP_NoDoublets_Reclustered_AfterHarmony_Res03.pdf", width = 13, height = 6)
p1 <- DimPlot(object = seuObject_slim_nodoub_withHarmony, reduction = "umap", label = TRUE, pt.size = 0.5) + theme(legend.position="none")
p2 <- DimPlot(object = seuObject_slim_nodoub_withHarmony, reduction = "umap", label = FALSE, pt.size = 0.5, group.by="Sample")
plot_grid(p1, p2)
dev.off()

save(seuObject_slim_nodoub_withHarmony, file = "output_sct_Integrated_Negative/07_SeuratObj_SCT_30pcs_03res_NoDoublet_Reclustered_withHarmony.RData")
...
ggsave("output_sct_Integrated_Negative/11_CellProportion_By_Sample.pdf", plot = prop_cell, width = 25, height = 15, units = "in", dpi = 150)

pdf("output_sct_Integrated_Negative/12_UMAP_Nodoublets_SplitBy_Sample.pdf", width = 10, height = 5)
