...
load("Negative/output_Relabel/08_Seurat_Positive_Final_Relabeled.RData")

dir.create("FinalPositive")
...
pdf("FinalPositive/08_UMAP_Labelled_Alternative.pdf", width = 7, height = 6)
...
pdf("FinalPositive/09_UMAP_Labelled_Alternative_withlegend_withaxis.pdf", width = 6, height = 5)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
...
pdf("FinalPositive/09_UMAP_Labelled_Alternative_withlegend_withaxis_splitby.pdf", width = 18, height = 6)
df %>%
...
save(pos_submacro, file = "FinalPositive/04_SubsetMacro_Reclustered.RData")

pdf("FinalPositive/10_Macro_SubsetUMAP.pdf", width = 8, height = 4)
...
pdf("FinalPositive/11_Macro_SubsetUMAP_Splitby_Genotype.pdf", width = 12, height = 4)
...
openxlsx::write.xlsx(all_markers.Sign, 
                     file = "FinalPositive/Macro_MarkersbyCluster_padjLT05_logfcGT0.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Markers")
...
pdf("FinalPositive/13_Macro_SubsetUMAP_Violin_1.pdf", width = 20, height = 15)
...
# Make the plot
pdf("FinalPositive/14_Macro_SubsetUMAP_DonutPlot.pdf", width = 12, height = 4)
p1 + p3 + p2
dev.off()
...
pdf("FinalPositive/15_Macro_FinalUMAP_withlegend_withaxis.pdf", width = 5, height = 4)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
...
pdf("FinalPositive/16_Macro_FinalUMAP_withlegend_withaxis_splitby.pdf", width = 18, height = 6)
df %>%
  mutate(Geno_ordered=fct_relevel(Sample,geno_order)) %>%
  ggplot(aes(x=UMAP_1, y=UMAP_2)) +
