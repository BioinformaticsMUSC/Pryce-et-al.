...
S71 <- Read10X_h5("cellbender_out/S71_filtered.h5", use.names = TRUE, unique.features = TRUE)
S73 <- Read10X_h5("cellbender_out/S73_filtered.h5", use.names = TRUE, unique.features = TRUE)
S75 <- Read10X_h5("cellbender_out/S75_filtered.h5", use.names = TRUE, unique.features = TRUE)

S71_obj <- CreateSeuratObject(counts = S71,min.features = 100)
S73_obj <- CreateSeuratObject(counts = S73,min.features = 100)
S75_obj <- CreateSeuratObject(counts = S75,min.features = 100)

S71_obj@meta.data$Sample <- "Pos_WS_PDAC"
S73_obj@meta.data$Sample <- "Pos_Control"
S75_obj@meta.data$Sample <- "Pos_C_PDAC"

seuObject <- merge(S71_obj,
                   y = c(S73_obj,S75_obj),
                   add.cell.ids = c("S71","S73","S75"),
                   project = "Gutt_human")
...
dir.create("output_sct_Integrated_Positive")

save(seuObject,file="output_sct_Integrated_Positive/01_SeuratObj_Unfilt.RData")

##########STEP7#############check data with plots
#QC plot 1
pdf("output_sct_Integrated_Positive/01_Quality_Control_plots.pdf", width=6,height=4)
feats <- c("nUMI", "nGene", "pMito")
VlnPlot(seuObject, group.by = "Sample", features = feats, pt.size = 0, ncol = 3) + 
  NoLegend()
dev.off()

#QC plot 2
pdf("output_sct_Integrated_Positive/02_Quality_Control_Scatter.pdf", width=5,height=4)
FeatureScatter(seuObject, "nUMI", "nGene", group.by = "Sample", pt.size = 0)
dev.off()
...
seuObject_final <- CreateSeuratObject(counts = mito_filtered, project = "Gutt_human")
...
save(seuObject_final,file="output_sct_Integrated_Positive/02_SeuratObj_final.RData")

seuObject_filt <- subset(x = seuObject_final, subset = nUMI < 30000 & pMito < 20)

save(seuObject_filt,file="output_sct_Integrated_Positive/02_SeuratObj_Filt.RData")

#Gene Expression Barplot
df <- table(seuObject_filt$Sample) %>% as.data.frame()

pdf("output_sct_Integrated_Positive/06_GeneExp_barplot.pdf",width = 5,height = 5)
...
#running Integration in cluster
seuObject_split <- SplitObject(seuObject_filt, split.by = "Genotype")

seuObject_split <- seuObject_split[c("S71","S73","S75")]
...
save(seuObject_integrated, file = "output_sct_Integrated_Positive/03_SeuratObj_Integrated_allres.RData")

pdf("output_sct_Integrated_Positive/07_Data_Integrated_Clustree.pdf", width = 12, height = 6)
...
save(seuObject_integrated, file = "output_sct_Integrated_Positive/04_SeuratObj_SCT_30pcs_05res.RData")

pdf("output_sct_Integrated_Positive/08_UMAP_Data_Integrated.pdf", width = 10, height = 6)
...
save(seuObject_integrated_slim, file = "output_sct_Integrated_Positive/05_SeuratObj_SCT_30pcs_05res_Slimmed.RData")
