...
load("output_Relabel_Negative/10_SeuratObj_SCT_30pcs_04res_labelled_WithCellCycle.RData")

table(negSeurat$Cell)

###2. Load Labels data
Labels <- read.table("output_sct_Integrated_Negative/New_Labels.txt",header=T,sep="\t")
...
save(negSeurat,file = "output_Relabel_Negative/11_FinalRelabel.RData")

pdf("output_Relabel_Negative/01_UMAP_Labelled.pdf", width = 6, height = 7)
DimPlot(object = negSeurat, reduction = "umap", label = TRUE, pt.size = 0.5) + theme(legend.position="none")
dev.off()

pdf("output_Relabel_Negative/02_UMAP_Labelled_Groupby.pdf", width = 12, height = 7)
...
pdf("output_Relabel_Negative/03_UMAP_Labelled_withclusters.pdf", width = 12, height = 7)
...
pdf("output_Relabel_Negative/04_UMAP_Labelled_Splitby_Genotype.pdf", width = 12, height = 7)
