...
save(seuObject_integrated_slim, file = "output_sct_Integrated_Positive/05_SeuratObj_SCT_withDoublets.RData")

df <- seuObject_integrated_slim@meta.data %>% as.data.frame()
tmp <- table(df$Doublets,df$Genotype) %>% as.data.frame()

pdf("output_sct_Integrated_Positive/09_GeneExp_Singlet-Doublet.pdf")
...
pdf("output_sct_Integrated_Positive/10_UMAP_NoDoublet.pdf", width = 10, height = 6)
...
save(seuObject_slim_nodoub_slim, file = "output_sct_Integrated_Positive/07_SeuObject_SCT_30pcs_allres_NoDoublet_slim.RData")

load("/Users/SuganyaSubramanian/BenKpp/output_sct_Integrated_Positive/07_SeuObject_SCT_30pcs_allres_NoDoublet_slim.RData")
...
openxlsx::write.xlsx(all_markers.Sign, 
                     file = "output_sct_Integrated_Positive/Res04_Presto_Filteredmarkers_padjLT05_logfcGT0.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Markers")
