...
##########################################
 ##   Read Cellbener out files         ##
##########################################

CtrlPos <- Read10X_h5("cellbender_out/Ctrl_Pos_filtered.h5", use.names = TRUE, unique.features = TRUE)
KppPos <- Read10X_h5("cellbender_out/Kpp_Pos_filtered.h5", use.names = TRUE, unique.features = TRUE)

##########################################
 ##      create seurat object          ##
##########################################

CtrlPos_obj <- CreateSeuratObject(counts = CtrlPos,min.features = 100)
KppPos_obj <- CreateSeuratObject(counts = KppPos,min.features = 100)

##########################################
 ##      Merge seurat objects         ##
##########################################
seuObject <- merge(CtrlPos_obj,
                   y = c(KppPos_obj),
                   add.cell.ids = c("CtrlPos","KppPos"),
                   project = "BenKpp")
...
pdf("output_sct_Integrated_Positive/01_Quality_Control_plots_filtered.pdf", width=6,height=4)
feats <- c("nUMI", "nGene", "pMito")
...
seuObject_split <- seuObject_split[c("CtrlPos","KppPos")]
...
save(seuObject_integrated, file = "output_sct_Integrated_Positive/03_SeuObj_Integrated_allres.RData")
