...
load("output_sct_Integrated_Negative/05_SeuratObj_SCT_30pcs_05res_Slimmed.RData")
...
save(seuObject_integrated_slim, file = "output_sct_Integrated_Negative/05_SeuratObj_SCT_withDoublets.RData")

df <- seuObject_integrated_slim@meta.data %>% as.data.frame()
tmp <- table(df$Doublets,df$Genotype) %>% as.data.frame()

pdf("output_sct_Integrated_Negative/09_GeneExp_Singlet-Doublet.pdf")
...
save(seuObject_slim_nodoub, file = "output_sct_Integrated_Negative/06_SeuratObj_SCT_30pcs_03res_NoDoublet_Reclustered.RData")

#load("output_sct_Integrated_Negative/06_SeuratObj_SCT_30pcs_03res_NoDoublet_Reclustered.RData")
...
pdf("output_sct_Integrated_Negative/10_UMAP_NoDoublet_res02.pdf", width = 10, height = 6)
p1 <- DimPlot(object = seuObject_slim_nodoub_slim, group.by = "SCT_snn_res.0.2",reduction = "umap", label = TRUE, pt.size = 0.5) + theme(legend.position="none")
p2 <- DimPlot(object = seuObject_slim_nodoub_slim, reduction = "umap", label = FALSE, pt.size = 0.5, group.by="Genotype")
plot_grid(p1, p2)
dev.off()

pdf("output_sct_Integrated_Negative/10_UMAP_NoDoublet_res03.pdf", width = 10, height = 6)
p1 <- DimPlot(object = seuObject_slim_nodoub_slim, group.by = "SCT_snn_res.0.3",reduction = "umap", label = TRUE, pt.size = 0.5) + theme(legend.position="none")
p2 <- DimPlot(object = seuObject_slim_nodoub_slim, reduction = "umap", label = FALSE, pt.size = 0.5, group.by="Genotype")
plot_grid(p1, p2)
dev.off()

pdf("output_sct_Integrated_Negative/10_UMAP_NoDoublet_res04.pdf", width = 10, height = 6)
p1 <- DimPlot(object = seuObject_slim_nodoub_slim, group.by = "SCT_snn_res.0.4", reduction = "umap", label = TRUE, pt.size = 0.5) + theme(legend.position="none")
p2 <- DimPlot(object = seuObject_slim_nodoub_slim, reduction = "umap", label = FALSE, pt.size = 0.5, group.by="Genotype")
plot_grid(p1, p2)
dev.off()
...
pdf("output_sct_Integrated_Negative/10_UMAP_NoDoublet.pdf", width = 10, height = 6)
...
save(seuObject_slim_nodoub_slim, file = "output_sct_Integrated_Negative/07_SeuObject_SCT_30pcs_04res_NoDoublet_slim.RData")
...
openxlsx::write.xlsx(all_markers.Sign, 
                     file = "output_sct_Integrated_Negative/Res04_Presto_Filteredmarkers_padjLT05_logfcGT0.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Markers")
