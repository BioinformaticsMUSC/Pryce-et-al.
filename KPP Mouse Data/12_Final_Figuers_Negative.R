...
write.table(col, "Negative/output_Relabel/Colors_Used.txt", sep="\t",quote=F)
...
pdf("Negative/output_Relabel/08_UMAP_Labelled_Alternative.pdf", width = 7, height = 6)
...
pdf("Negative/output_Relabel/09_UMAP_Labelled_Alternative_withlegend_withaxis.pdf", width = 6, height = 5)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
...
pdf("Negative/output_Relabel/09_UMAP_Labelled_Alternative_withlegend_withaxis_splitby.pdf", width = 12, height = 6)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
  ggrastr::geom_point_rast(aes(colour = Cell),size=0.1) +
  #scale_color_viridis(discrete=TRUE,option="inferno")
  #scale_colour_manual(values = colorRampPalette(brewer.pal(13, "Paired"))(31))
...
save(neg_subfaps, file = "Negative/output_Relabel/04_SubsetFaps_Reclustered.RData")
load("Negative/output_Relabel/04_SubsetFaps_Reclustered.RData")
...
pdf("Negative/output_Relabel/10_Faps_SubsetUMAP.pdf", width = 10, height = 6)
...
pdf("Negative/output_Relabel/11_Faps_SubsetUMAP_Splitby_Genotype.pdf", width = 12, height = 7)
...
openxlsx::write.xlsx(all_markers.Sign, 
                     file = "Negative/output_Relabel/Faps_MarkersbyCluster_padjLT05_logfcGT0.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Markers")
...
pdf("Negative/output_Relabel/12_Faps_SubsetUMAP_ViolinStacked.pdf", width = 10, height = 20)
VlnPlot(neg_subfaps, genes, stack = TRUE, sort = TRUE, flip = TRUE) +
  theme(legend.position = "none") + ggtitle("Identity on x-axis")
dev.off()

pdf("Negative/output_Relabel/13_Faps_SubsetUMAP_Violin_1.pdf", width = 20, height = 15)
VlnPlot(neg_subfaps, genes,ncol = 3,pt.size = 0)
dev.off()

#requested on Oct10
genes <- c("Mmp3", "Plau", "Tgif1", "Myc")
pdf("Negative/output_Relabel/13_Faps_SubsetUMAP_Violin_2.pdf", width = 20, height = 5)
...
# Make the plot
pdf("Negative/output_Relabel/14_Faps_SubsetUMAP_DonutPlot.pdf", width = 8, height = 4)
p1 + p2
dev.off()
...
pdf("Negative/output_Relabel/15_FAPs_FinalUMAP_withlegend_withaxis.pdf", width = 5, height = 4)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
...
pdf("Negative/output_Relabel/15_FAPs_FinalUMAP_withlegend_withaxis_splitby.pdf", width = 10, height = 4)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
