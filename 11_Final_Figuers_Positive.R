...
posSeurat@meta.data$Cell <- factor(posSeurat@meta.data$Cell, levels = order)
...
#write.table(col, "Positive/output_Relabel/Colors_Used.txt", sep="\t",quote=F)
...
#myeliod dc #E75B40
#platelets #6CE573
#col <- c("#7FE5E1","#7EE6B8","#E4A6A0","#80A878",
#         "#E07647","#94C2EC","#7BED43","#D1DC58",
#         "#A8EC3D","#69D4D9","#E75B40","#DFE69C",
#         "#6CE573")

#running with defaults color


pdf("Positive/output_Relabel/08_UMAP_Labelled_Alternative.pdf", width = 7, height = 6)
...
pdf("Positive/output_Relabel/09_UMAP_Labelled_Alternative_withlegend_withaxis.pdf", width = 6, height = 5)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
...
pdf("Positive/output_Relabel/09_UMAP_Labelled_Alternative_withlegend_withaxis_splitby.pdf", width = 12, height = 6)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
  ggrastr::geom_point_rast(aes(colour = Cell),size=0.3) +
  #scale_color_viridis(discrete=TRUE,option="inferno")
  #scale_colour_manual(values = colorRampPalette(brewer.pal(13, "Paired"))(31))
  #scale_colour_manual(values = met.brewer(name="Klimt",n=13))+    
  #scale_colour_manual(values = col)+
...
#Subset Macrophage
pos_submacro <- subset(posSeurat, subset = Cell == "Macrophage")
...
save(pos_submacro, file = "Positive/output_Relabel/04_SubsetMacrophage_Reclustered.RData")
load("Positive/output_Relabel/04_SubsetMacrophage_Reclustered.RData")
...
Idents(pos_submacro) <- "SCT_snn_res.0.4"
pos_submacro@meta.data$seurat_clusters <- pos_submacro@meta.data$SCT_snn_res.0.4

pdf("Positive/output_Relabel/10_Macrophage_SubsetUMAP.pdf", width = 10, height = 6)
p1 <- DimPlot(object = pos_submacro, reduction = "umap", label = TRUE, pt.size = 0.5) + theme(legend.position="none")
p2 <- DimPlot(object = pos_submacro, reduction = "umap", label = FALSE, pt.size = 0.5, group.by="Genotype")
plot_grid(p1, p2)
dev.off()

pdf("Positive/output_Relabel/11_Macrophage_SubsetUMAP_Splitby_Genotype.pdf", width = 12, height = 7)
DimPlot(object = pos_submacro, reduction = "umap", label = TRUE,ncol = 2, pt.size = 0.5,split.by = "Genotype") + theme(legend.position="none")
dev.off()
...
openxlsx::write.xlsx(all_markers.Sign, 
                     file = "Positive/output_Relabel/Macrophage_MarkersbyCluster_padjLT05_logfcGT0.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Markers")
...
genes <- c("Maf","Mef2c","Lyve1","Tcf4","Ccr2","S100a9")

pdf("Positive/output_Relabel/12_Macrophage_SubsetUMAP_ViolinStacked.pdf", width = 10, height = 20)
VlnPlot(pos_submacro, genes, stack = TRUE, sort = TRUE, flip = TRUE) +
  theme(legend.position = "none") + ggtitle("Identity on x-axis")
dev.off()

pdf("Positive/output_Relabel/13_Macrophage_SubsetUMAP_Violin_1.pdf", width = 20, height = 10)
...
df <- table(Idents(pos_submacro),pos_submacro$Genotype)
df <- as.data.frame(df)

df_ctrl <- df[df$Var2=="CtrlPos",]
...
# Make the plot
pdf("Positive/output_Relabel/14_Macrophage_SubsetUMAP_DonutPlot.pdf", width = 8, height = 4)
p1 + p2
dev.off()


##### final macrophage UMAPS
...
pdf("Positive/output_Relabel/15_Macrophage_FinalUMAP_withlegend_withaxis.pdf", width = 5, height = 4)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
...
pdf("Positive/output_Relabel/15_Macrophage_FinalUMAP_withlegend_withaxis_splitby.pdf", width = 10, height = 4)
ggplot(df, aes(x=UMAP_1, y=UMAP_2)) +
