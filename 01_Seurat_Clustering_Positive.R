...
dir.create("output_sct_Integrated")

save(seuObject,file="output_sct_Integrated/01_SeuratObj_Unfilt.RData")

##########################################
 ##            QC plots               ##
##########################################
pdf("output_sct_Integrated/01_Quality_Control_plots.pdf", width=6,height=4)
...
save(seuObject_final,file="output_sct_Integrated/02_SeuratObj_final.RData")

seuObject_filt <- subset(x = seuObject_final, subset = nUMI < 30000 & pMito < 20)

pdf("output_sct_Integrated/01_Quality_Control_plots_filtered.pdf", width=6,height=4)
feats <- c("nUMI", "nGene", "pMito")
VlnPlot(seuObject_filt, group.by = "Genotype", features = feats, pt.size = 0, ncol = 3) + 
  NoLegend()
dev.off()

save(seuObject_filt,file="output_sct_Integrated/02_SeuratObj_Filt.RData")

#Gene Expression Barplot
df <- table(seuObject_filt$Genotype) %>% as.data.frame()

pdf("output_sct_Integrated/06_GeneExp_barplot.pdf",width = 5,height = 5)
...
save(seuObject_integrated, file = "output_sct_Integrated/03_SeuObj_Integrated_allres.RData")

pdf("output_sct_Integrated/07_Data_Integrated_Clustree.pdf", width = 12, height = 6)
...
save(seuObject_integrated, file = "output_sct_Integrated/04_SeuratObj_SCT_30pcs_05res.RData")


pdf("output_sct_Integrated/08_UMAP_Data_Integrated.pdf", width = 10, height = 6)
...
save(seuObject_integrated_slim, file = "output_sct_Integrated/05_SeuratObj_SCT_30pcs_05res_Slimmed.RData")
